...
def grade(db, user_id: str, answers: dict[str, int]) -> dict:
    ...
    Aufrufer hat onboarded_at bereits atomar geclaimt (claim_onboarding) → läuft genau
    einmal. Jede Frage ist isoliert: scheitert eine Aussaat, macht der Rest trotzdem weiter
    (keine app-übergreifende Transaktion in PostgREST; die States sind idempotent-nah)."""
    import logging
    ...
    for q in QUESTIONS:
        ...
        try:
            ...
            logger.exception("onboarding: Frage '%s' konnte nicht gesät werden", q["id"])

    level = _estimate_level({b: tuple(v) for b, v in scores.items()})
    db.update_user(user_id, {"level_estimate": level})   # onboarded_at ist schon geclaimt
