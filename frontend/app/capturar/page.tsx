 ...
 * La respuesta es una microdosis (corrección + una frase de porqué) y el resto se
 * archiva en silencio. Deep-link: /capturar?mode=camera|voz|texto
 */
...
type CaptureResult = {
  mode: string;
  gist: string | null;
  correction: { wrong: string; correct: string; why: string; concept_slug: string } | null;
  notes: string;
};
  ...
  return (
    <>
      ...
      <textarea
        ...
        rows={4}
        className="w-full rounded-xl border border-stone-300 bg-white p-4 text-base outline-none focus:border-amber-500"
      />
      ...
      {/* La microdosis — corrección + una frase. Nada de lección completa. */}
      {result && (
        <div className="mt-6 rounded-xl border border-stone-200 bg-white p-4">
          ...
          {result.gist && <p className="mb-2 text-base">{result.gist}</p>}
